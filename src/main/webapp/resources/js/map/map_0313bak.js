...
var useMapBounds = false; // useMapBounds 사용여부
...
$(function(){
	...
	if(searchList){
		...
		setKeywordAndSearchPlace(region + " " + town + " " + hospitalNm, false);
	}
});
...
function setKeywordAndSearchPlace(keyword, useMapBounds){
	document.getElementById('keyword').value = keyword;
	document.getElementById('useMapBounds').checked = useMapBounds;
...
function placesSearchCB(data, status, pagination) {
    if (status === kakao.maps.services.Status.OK) {
        ...
        displayPlaces(data);
        // 페이징
        if(searchList)displayPagination(pagination);
    ...
    for ( var i=0; i<places.length; i++ ) {
        ...
        (function(marker, places, placePosition) {
            ...
            itemEl.onclick =  function () {
            	...
            	if(wCatch()=='m' || wCatch()=='t'){
            		initPlacesList(false);
...
// 	    '   <a class="title" href="https://map.kakao.com/link/to/' + place.place_name + ',' + place.y + ',' + place.x + '" target="_blank" title="' + place.place_name + '">' + place.place_name + '</a>';   
	...
	'<a class="btn_close" onclick="placeOverlay.setMap(null)"><span class="hdn">닫기</span></a><a class="btn_roadmap"><span>길찾기</span></a></div>' + 
	'<div class="after"></div>';
